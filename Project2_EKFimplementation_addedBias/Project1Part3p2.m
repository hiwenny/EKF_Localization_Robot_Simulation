...
%     CreateFigureToPlot(25) ;        % figure inits
    ...
    P = diag( [0, 0, 0, 0] );            % initial quality --> perfect (covariance =zero, bias speed measurement )
    ...
    while 1,
        ...
        gyroZ = mean(r.data(6,1:r.n)) - biasGyro;
        bg = biasGyro*180/pi
            ...
            if ~isempty(index_OOIdetected) && ~isempty(local_to_global_X)
                ...
                for foo_u = 1:length(index_OOIdetected)
                    ...
                    if length(OOIdetected) <= length(index_OOIdetected)                        
                        ...
                        eDX = (globalMap.refOOI(ID,1)-Xe(1)) ;  % (xu-x)
                        eDY = (globalMap.refOOI(ID,2)-Xe(2)) ;           % (yu-y)
                        % Range input
