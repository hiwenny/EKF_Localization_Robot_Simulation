% Part2.m - version modified of DemoEKF.m 
...
% This way the EKF accepts input from both range and bearing
% It can be noted that the EKF trajectory is closer to the real one when
% using both inputs. Here I tried making the standard deviation 30 deg/sec
% and 1 m (super big) yet it still shows a good estimation.
...
P = diag( [0, 0, 0, (4*pi/180)^2] );            % initial quality --> perfect (covariance =zero )
...
function InitSimulation(stdDevSpeed,stdDevGyro,sdev_bearingMeasurement,sdev_rangeMeasurement,DtObservations)
    ...
    ContextSimulation.biasConstant = 1*pi/180;
return;
...
subplot(414) ; plot(1-Xe_History(4,:)*180/pi); ylabel('real - estimated bias (deg/s)') ;
