...
%     biasGyro = 0;
    ...
    while 1,
        ...
        gyroZ = mean(r.data(6,1:r.n)) - Xe(4);
        ...
        if s.n>0,
            if iterations<=calib_scans            
                ...
                if iterations == calib_scans                   
                    ...
                    globalMap.refOOI = [globalMap.refOOI(:,1)+0.5 globalMap.refOOI(:,2)+0.5];
                    globalMap.N = OOIs.N;
...
%                     biasGyro = mean(buffer_OOI.biasGyro);   % rad/s
    ...
    %         break;
                ...
                local_to_global_X = cos(rotationAngle)*OOIs.Centers(:,1) - sin(rotationAngle)*OOIs.Centers(:,2) + Xe(1);
                local_to_global_Y = sin(rotationAngle)*OOIs.Centers(:,1) + cos(rotationAngle)*OOIs.Centers(:,2) + Xe(2);

                % Rotation Matrix for all points (map)
                local_to_global_Map_X = cos(rotationAngle)*Laser_xx - sin(rotationAngle)*Laser_yy + Xe(1);
                local_to_global_Map_Y = sin(rotationAngle)*Laser_xx + cos(rotationAngle)*Laser_yy + Xe(2);
            ...
            if ~isempty(index_OOIdetected) && ~isempty(local_to_global_X)
                ...
                for foo_u = 1:length(index_OOIdetected)
                    ...
                    if length(OOIdetected) <= length(index_OOIdetected)                        
                        ...
                        RangeH = (2*d*sin(Xe(3))*eDX - 2*d*cos(Xe(3))*eDY)/(2*sqrt((eDX^2)+(eDY^2)) );
                        BearingH = (((-(d*sin(Xe(3))*eDX)/eDY^2)- (d*cos(Xe(3))/eDY))/((eDX^2)/(eDY^2)+1))-1;
                        
                        % here is it. "H". I reuse some previous calculations.
                        H_range = [  -eDX./eDD_range , -eDY./eDD_range , RangeH , 0 ] ;   % Jacobian of h(X),range
                        H_bearing = [ eDY./eDD_bearing, -eDX./eDD_bearing, BearingH , 0 ];
                        H = [H_range; H_bearing];
...
function output = ProcessMyImuData(r, CurrAttitude) 
    % r a structure containing measured att acceleration, CurrAttitude the previous att
    % updates the att
    % only gyro Z used in this part 3 implementation
...
%     foo_a = find(att < -360);
%     att(foo_a) = att(foo_a) + 360;
%     foo_b = find(att > 360);
%     att(foo_b) = att(foo_b) - 360;
    output = att; % deg
...
%    % Handles for local coord plotting
%    figure(k); clf; hold on;
%    MyContext.h1 = plot(0,0,'.') ;   % all points
%    MyContext.h2 = plot(0,0,'*r') ;  % OOIs
%    axis([-1000,1000,0,1000]);
%    zoom on; xlabel('x (cm) [Cartesian]') ; ylabel('y (cm)') ;
%    hold off;
%    
%    % Handles for global coord plotting
%    figure(k+1); clf; hold on;   
%    MyContext.h3 = plot(0,0,'*r') ;  % ref OOIs
%    MyContext.h4 = plot(0,0,'ob') ;  % seen OOIs   
%    MyContext.h5 = plot(0,0,'.g') ;  % robot path
%    MyContext.h7 = plot(0,0,'.b') ;  % ref Map
%    MyContext.h6 = quiver(0,0,0,0,'og'); % global posn of robot
%    axis([-1000,1000,0,1000]);
%    zoom on; xlabel('x (cm) [Cartesian]') ; ylabel('y (cm)') ;
%    hold off;
return;
...
%     set(MyContext.h2,'xdata',OOIs.Centers(:,1),'ydata',OOIs.Centers(:,2));
% return;
% end
